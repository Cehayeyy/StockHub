...
class StokHarianController extends Controller
{
    public function bar(Request $request)
    {
        ...
        return Inertia::render('StokHarian/Bar', [
            ...
            'search'         => $search,
            'canInput'       => $this->canUserInput($tanggal),
            'isPastCutoff'   => Carbon::now()->greaterThan(Carbon::today()->setTime(21, 0, 0)),
            'search'         => $search,
        ]);
    }
  ...
  private function canUserInput($tanggal) {
    ...
    // 1. CEK APAKAH SUDAH INPUT (Data sudah di-submit)
    // Kita cek apakah ada record di StokHarianMenu untuk tanggal ini yang is_submitted-nya true
    $alreadySubmitted = StokHarianMenu::whereDate('tanggal', $tanggal)
                        ->where('is_submitted', 1)
                        ->exists();

    // Jika sudah pernah input/simpan, langsung kembalikan false (tombol jadi abu-abu)
    // kecuali jika staff punya izin revisi yang masih berlaku.
    if ($alreadySubmitted) {
        ...
        return $hasIzin || in_array($user->role, ['owner', 'supervisor']);
    }

    // 2. CEK BATASAN WAKTU (Logic jam 9 malam kamu)
    if (in_array($user->role, ['owner', 'supervisor'])) return true;

    $now = Carbon::now();
    $cutoffTime = Carbon::today()->setTime(21, 0, 0);

    // Jika belum jam 9 malam, staff boleh input
    if ($now->lessThan($cutoffTime)) return true;

    // 3. CEK IZIN REVISI (Jika sudah lewat jam 9 malam)
    return IzinRevisi::where('user_id', $user->id)
            ->where('status', 'approved')
