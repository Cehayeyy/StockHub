...
export default function AnalisaProfit({ ringkasan, itemsAnalisa, filters }: Props) {
    ...
    const totalTerjualCount = itemsAnalisa.reduce((acc, curr) => acc + curr.terjual, 0);
    const totalHppSum = itemsAnalisa.reduce((acc, curr) => acc + curr.total_hpp, 0);
    const totalProfitSum = itemsAnalisa.reduce((acc, curr) => acc + curr.total_profit_real, 0);
            ...
            <div className="py-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white p-6 rounded-3xl shadow-xs border border-gray-100 flex items-center justify-between">
                        <div>
                            <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">TOTAL OMSET REAL / GROSS</p>
                            <p className="text-2xl font-black text-gray-800 mt-1">
                                {formatRupiah(ringkasan?.totalOmsetReal || 0)}
                            </p>
                            <p className="text-[11px] font-medium text-gray-400 mt-1">Berdasarkan Harga Jual Real (HJR)</p>
                        </div>
                ...
                <div className="bg-[#F2ECE4] p-4 md:p-6 rounded-3xl shadow-sm border border-amber-200/60 flex flex-col">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-5 gap-3">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="bg-[#8B5E3C]/10 text-[#8B5E3C] px-3 py-1 rounded-full text-xs font-bold">
                                {itemsAnalisa.length} Menu Terdaftar
                            </span>
                        </div>
                            ...
                            <table className="w-full text-left text-xs whitespace-nowrap">
                                <thead className="bg-[#FAF7F2] text-gray-500 font-bold uppercase tracking-wider border-b border-gray-100">
                                    <tr>
                                        ...
                                        <th className="py-4 px-5 text-right w-36">Harga Jual Riil (HJR)</th>
                                ...
                                <tbody className="divide-y divide-gray-50">
                                    {itemsAnalisa && itemsAnalisa.length > 0 ? (
                                        itemsAnalisa.map((item, index) => (
                                            <tr key={item.id} className="hover:bg-[#FDF3E4]/50 transition-colors">
                                                <td className="py-3.5 px-5 text-center text-gray-400 font-medium">{index + 1}</td>
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-between text-xs text-gray-500 px-2 gap-3">
                        <span>Menampilkan 1-{itemsAnalisa.length} dari {itemsAnalisa.length} menu resep</span>
                        <div className="flex items-center gap-1">
                            <button className="p-1.5 rounded-xl border border-gray-200 bg-white text-gray-400 hover:bg-gray-50 disabled:opacity-50">
                                <ChevronLeft size={16} />
                            </button>
                            <button className="px-3 py-1 rounded-xl bg-[#8B5E3C] text-white font-bold shadow-xs">1</button>
                            <button className="p-1.5 rounded-xl border border-gray-200 bg-white text-gray-400 hover:bg-gray-50">
