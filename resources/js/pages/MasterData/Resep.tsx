...
const Resep: React.FC = () => {
  ...
  const saveRecipe = (e: React.FormEvent) => {
    ...
    // Validasi untuk BAR: Menu harus ada di list Item
    if (division === 'bar' && !menu) {
  ...
  // ... (Sisa fungsi edit sama seperti add, disingkat untuk kejelasan) ...
  ...
  const addEditIngredient = () => setEditIngredients(prev => [...prev, { id: Date.now(), item_id: null, item_name: "", amount: 1, unit: "porsi" }]);
  const removeEditIngredient = (id: number) => setEditIngredients(prev => prev.filter(i => i.id !== id));
  ...
  return (
    <AppLayout header={`Resep ${selectedDivision === "bar" ? "Bar" : "Dapur"}`}>
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 min-h-[600px] flex flex-col">
          ...
          <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
             ...
             {!isStaff ? (
              <div className="relative inline-block w-40">
                ...
                >
                  ...
                  <ChevronDown className={`h-4 w-4 transition-transform ${showDivisionDropdown ? "rotate-180" : ""}`} />
                </button>
                {showDivisionDropdown && (
                  <div className="absolute left-0 mt-1 w-full rounded-2xl bg-[#E7BE8B] py-1 text-sm shadow-lg z-20 overflow-hidden">
                    <button onClick={() => changeDivision("bar")} className="block w-full px-4 py-2 text-left hover:bg-[#d9a978]">Bar</button>
                    <button onClick={() => changeDivision("dapur")} className="block w-full px-4 py-2 text-left hover:bg-[#d9a978]">Dapur</button>
                  </div>
                )}
              </div>
             ) : (
             ...
             <div className="flex gap-3 items-center">
                {!isStaff && (
                    <button onClick={() => setShowModal(true)} className="rounded-full bg-[#D9A978] px-5 py-2 text-sm font-bold text-white shadow-md hover:bg-[#c4925e]">
                        Tambah Resep
                    </button>
                )}
             ...
             <table className="w-full text-sm whitespace-nowrap text-left">
                ...
                <tbody className="divide-y divide-gray-100">
                    {filteredRecipes.length === 0 ? (
                        <tr><td colSpan={5} className="p-8 text-center text-gray-400">Belum ada resep.</td></tr>
                    ) : (
                            ...
                            <tr key={r.id} className="hover:bg-[#FFF7EC] transition">
                                ...
                                <td className="p-4 text-center">
                                    <div className="flex justify-center gap-2">
                                        {!isStaff && (
                                            <button onClick={() => openEdit(r)} className="px-3 py-1 bg-[#1D8CFF] text-white rounded-full text-xs font-semibold hover:bg-[#166ac4]">Edit</button>
                                        )}
                                        <button onClick={() => openViewRecipe(r)} className="px-3 py-1 bg-gray-500 text-white rounded-full text-xs font-semibold hover:bg-gray-600">View</button>
                                        {!isStaff && (
                                            <button onClick={() => openDeleteConfirm(r.id)} className="px-3 py-1 bg-[#FF4B4B] text-white rounded-full text-xs font-semibold hover:bg-[#e03535]">Hapus</button>
                                        )}
                                    </div>
                                </td>
                            </tr>
      ...
      {/* --- MODALS --- */}
      {/* (Modal Tambah, Edit, Delete, View kode tetap sama seperti sebelumnya, pastikan isStaff membatasi akses jika perlu) */}
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
            <div className="bg-white p-8 rounded-3xl w-full max-w-lg shadow-xl">
                <h3 className="text-xl font-bold text-center mb-6">Tambah Resep</h3>
                <form onSubmit={saveRecipe} className="space-y-4">
                    <div>
                        <label className="block text-sm font-bold text-gray-700 mb-1">Nama Menu Jadi</label>
                        <input
                            type="text"
                            list={division === 'bar' ? "menu-datalist" : undefined}
                            value={menuName}
                            onChange={(e) => setMenuName(e.target.value)}
                            className="w-full bg-gray-50 rounded-xl px-4 py-2 border border-gray-200"
                            placeholder="Ketik nama menu..."
                            required
                        />
                    </div>
                    {/* ... (Sisa form ingredients sama) ... */}
                    <div className="flex justify-end gap-3 mt-6">
                        <button type="button" onClick={() => setShowModal(false)} className="px-4 py-2 bg-gray-200 rounded-xl font-semibold text-gray-700">Batal</button>
                        <button type="submit" className="px-4 py-2 bg-[#D9A978] rounded-xl font-bold text-white">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
      ...
      {/* ... (Modal View, Edit, Delete code omitted for brevity but should be included) ... */}
