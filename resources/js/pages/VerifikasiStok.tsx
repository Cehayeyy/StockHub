...
import { Search, Calendar, CheckCircle, AlertCircle, Save } from "lucide-react";
    ...
    <AppLayout header="Verifikasi Stok Mingguan">
      ...
      <div className="py-6 space-y-6">
        ...
        <div className="bg-[#FFF9F0] border border-[#FDF3E4] p-4 rounded-2xl flex items-start gap-3 shadow-sm">
          <div className="p-2 bg-[#D9A978] rounded-full text-white">
          ...
          <div>
            ...
            <p className="text-[#A68B6C] text-sm mt-1">
              ...
              <br/>
              Sistem menampilkan data stok acuan dari hari <strong>Senin, {new Date(tanggal_data).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</strong>.
            </p>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 min-h-[500px]">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
            ...
            {/* TABS */}
            <div className="flex bg-[#FDF3E4] rounded-full p-1">
              <button onClick={() => handleTabSwitch("bar")} className={`px-6 py-2 rounded-full text-sm font-bold transition-all ${tab === "bar" ? "bg-[#D9A978] text-white shadow-sm" : "text-gray-500 hover:text-[#D9A978]"}`}>Bar</button>
              <button onClick={() => handleTabSwitch("dapur")} className={`px-6 py-2 rounded-full text-sm font-bold transition-all ${tab === "dapur" ? "bg-[#D9A978] text-white shadow-sm" : "text-gray-500 hover:text-[#D9A978]"}`}>Dapur</button>
            </div>
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                <input
                  type="date"
                  value={tanggal_picker} // Menggunakan tanggal pilihan user
                  onChange={handleDateChange}
                  className="bg-[#FDF3E4] border-none rounded-full px-4 py-2 text-sm text-[#8B5E3C] font-medium focus:ring-2 focus:ring-[#D9A978]"
                />
              </div>
              <div className="relative">
                <input
                  ...
                  className="bg-[#FDF3E4] border-none rounded-full pl-10 pr-4 py-2 text-sm w-64 focus:ring-2 focus:ring-[#D9A978]"
                />
